...
import HeatmapWithDynamicAxes from "./HeatmapWithDynamicAxes";

interface Props {
  data: any[];
}

const KpiMonitor: React.FC<Props> = ({ data }: Props) => {
  const grid = {
    left: 100,
    right: 100,
    top: 50,
    ...
    <>
      ...
      <Row gutter={[16, 16]} justify="center">
        <Col
          xs={24}
          md={12}
          style={{ background: "#fafafa", padding: "20px", height: "500px" }}
        >
          <Chart data={[]}></Chart>
        </Col>
        <Col xs={24} md={12} style={{ background: "#f0f0f0", padding: "20px" }}>
        <HeatmapWithDynamicAxes></HeatmapWithDynamicAxes>
        </Col>
      </Row>
      <div style={{ height: "1000px", width: "1200px" }}></div>
